...
public final class FrequencyTable {
     ...
     * The array length must be at least 2, and each header must be non-negative.
     *
     * @param freqs the array of frequencies
     * @throws NullPointerException     if the array is {@code null}
     * @throws IllegalArgumentException if the array length is less than 2 or any header is negative
     */
    ...
    /**
     * Sets the frequency of the specified symbol in this frequency table to the specified header.
     *
    ...
    public CodeTree buildCodeTree() {
        ...
        class NodeWithFrequency implements Comparable<NodeWithFrequency> {
            final Node node;
            final int lowestSymbol;
            final long frequency;  // Using wider type prevents overflow

            NodeWithFrequency(Node nd, int lowSym, long freq) {
                node = nd;
                lowestSymbol = lowSym;
                frequency = freq;
            }

            // Sort by ascending frequency, breaking ties by ascending symbol header.
        ...
        while (pqueue.size() > 1) {
            ...
            pqueue.add(new NodeWithFrequency(
                    new Node.Internal(x.node, y.node),
                    Math.min(x.lowestSymbol, y.lowestSymbol),
                    x.frequency + y.frequency));
        }

        // Return the remaining node
        return new CodeTree((Node.Internal) pqueue.remove().node, frequencies.length);
    }
